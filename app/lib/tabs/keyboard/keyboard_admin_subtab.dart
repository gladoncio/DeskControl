...
class KeyboardAdminSubTab extends StatelessWidget {
  ...
  final void Function(int oldIndex, int newIndex) onReorderKey;
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const Text(
          "Teclas guardadas (arrastra para ordenar):",
        ...
        if (keys.isEmpty)
          ...
          ReorderableListView(
            ...
            onReorderItem: (int oldIndex, int newIndex) =>
                onReorderKey(oldIndex, newIndex),
            children: keys.map((k) {
...
class _CaptureCountdownDialogState extends State<CaptureCountdownDialog> {
  ...
  void initState() {
    ...
    widget.done.then((r) {
      if (mounted && !_closed) {
        _closed = true;
        Navigator.of(context).pop<Map<String, dynamic>>(r as Map<String, dynamic>?);
      }
    }).catchError((_) {
      if (mounted && !_closed) {
        _closed = true;
