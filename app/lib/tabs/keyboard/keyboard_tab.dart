...
class KeyboardTab extends StatefulWidget {
  ...
  const KeyboardTab({super.key, required this.desk, required this.profileId});
...
class _KeyboardTabState extends State<KeyboardTab> {
  ...
  Future<void> _loadAll() async {
    ...
    final k = await AppStorage.loadKeys(profileId: widget.profileId);
    final c = await AppStorage.loadCombos(profileId: widget.profileId);
    final vis = await AppStorage.loadVisibleKeyIds(profileId: widget.profileId);
    ...
    final vset = <String>{...vis};
      ...
      await AppStorage.saveVisibleKeyIds(vset.toList(), profileId: widget.profileId);
    }
  ...
  Future<void> _saveKeys() async =>
      AppStorage.saveKeys(keys, profileId: widget.profileId);
  Future<void> _saveCombos() async =>
      AppStorage.saveCombos(combos, profileId: widget.profileId);
  Future<void> _saveVisible() async =>
      AppStorage.saveVisibleKeyIds(visibleKeyIds.toList(), profileId: widget.profileId);
  ...
  Future<void> _addKeyCaptureVK() async {
    final captura = widget.desk.request(
      "capture_start",
      payload: {"timeout_ms": 10000},
    );

    final resp = await showDialog<Map<String, dynamic>>(
      context: context,
      barrierDismissible: false,
      builder: (ctx) => CaptureCountdownDialog(
        seconds: 10,
        done: captura,
        onCancel: () {},
      ),
    ...
    final keyData = (resp['result'] is Map)
        ? (resp['result'] as Map)['key']
        : resp['key'];
    if (keyData is! Map) return;

    final vk = (keyData['vk'] as num?)?.toInt() ?? 0;
    final scan = (keyData['scan'] as num?)?.toInt() ?? 0;
    final ext = (keyData['ext'] == true);
    ...
    final item = KeyItem(
      ...
      name: name,
      useVK: true,
      keySpec: KeySpec(vk: vk, scan: scan, ext: ext),
      keyName: null,
    );
